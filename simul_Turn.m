...
EXE_TIME = 3                              % Application running time in seconds
...
while stat == true                          % Quit when times up
    ...
    uB_k=uB_k1+delta_uB;
    fprintf('A.speed=%f, B.speed=%f', uA_k, uB_k)
...
%% plot
subplot(2, 1, 1);
plot(TIME_STEP, POSITION_CMD(1, :), 'rx', TIME_STEP, POSITION_OUT(1, :), 'bx');
title('Right wheel')
xlabel('time/s'); ylabel('rotation/degree');
legend('position cmd','position out')
subplot(2, 1, 2);
plot(TIME_STEP, POSITION_CMD(2, :), 'rx', TIME_STEP, POSITION_OUT(2, :), 'bx');
title('Left wheel')
xlabel('time/s'); ylabel('rotation/degree');
legend('position cmd','position out')